        ...
        public static WitResponseClass GetPostData(string endpoint, Dictionary<string, string> parameters)
        {
            ...
            if (parameters != null)
            {
                foreach (var key in parameters.Keys)
                {
                    ...
                    if (string.Equals(key, WitConstants.ENDPOINT_CONTEXT_PARAM))
                    {
                        rootNode[key] = JsonConvert.DeserializeToken(val);
                        continue;
                    }
                    if (val[0].Equals(WitConstants.WIT_SOCKET_PARAM_START)
                        && val[^1].Equals(WitConstants.WIT_SOCKET_PARAM_END))
                    {
                        val = val.Substring(1, val.Length - 2);
                        var newArray = new WitResponseArray();
                        var splitVals = val.Split(WitConstants.WIT_SOCKET_PARAM_DELIM);
                        for (int i = 0; i < splitVals.Length; i++)
                        {
                            newArray[i] = new WitResponseData(splitVals[i]);
                        }
                        parameterNode[key] = newArray;
                        continue;
                    }
                    parameterNode[key] = new WitResponseData(val);
                }
            }
