...
namespace Meta.WitAi
{
    ...
    public static class WitRequestSettings
    {
        ...
        private static void Init()
        {
            ...
            {
                LocalClientUserId = PlayerPrefs.GetString(PREF_CLIENT_USER_ID);
            }
        }
        #if UNITY_EDITOR
        /// <summary>
        ...
        public static string LocalClientUserId
        {
            ...
            set
            {
                ...
                {
                    newVal = System.Guid.NewGuid().ToString();
                }
                // Ignore if same
                else if (string.Equals(newVal, _localClientUserId))
                {
                    return;
                }
                // Otherwise, set
                _localClientUserId = newVal;
        ...
        /// <summary>
        /// Maps to newer version with full options class for backwards compatibility
        /// </summary>
        public static Dictionary<string, string> GetHeaders(IWitRequestConfiguration configuration, string requestId,
          bool useServerToken, string clientUserId = null) =>
          GetHeaders(configuration, new WitRequestOptions(requestId, clientUserId, null, null), useServerToken);
